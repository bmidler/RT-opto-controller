...
//
//  3) Pulse reporting (board -> PC):
//       Every time a laser pulse STARTS (the gate pin goes HIGH), the board
//       echoes a marker line back over USB so the controller can record the
//       ACTUAL stim time instead of inferring it:
//
//         P,<pulseIndex>,<micros>\n
//
//       pulseIndex : 0-based pulse number within the current activation
//       micros     : board micros() at the rising edge (for jitter checks)
//       The PC timestamps the marker's arrival against its own clock; that PC
//       time is what aligns each pulse to the recorded video.
//
...
// Report a pulse's rising edge back to the PC. Single short line, written to
// the (buffered, non-blocking) USB serial -- negligible next to the pulse
// period at any usable stim frequency, so it does not perturb pulse timing.
void emitPulseMark() {
  Serial.print('P');
  Serial.print(',');
...
void loop() {
  ...
  if (stimulating) {
    ...
    if (elapsed >= periodUs) {
      ...
      emitPulseMark();                          // report this pulse's edge
