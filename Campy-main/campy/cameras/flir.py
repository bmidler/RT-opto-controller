...
def OpenCamera(cam_params):
	...
	# Reset to factory default settings. NOTE: are you sure you want default setting? Propper config is usually UserSet0.
	# camera.UserSetSelector.SetValue(PySpin.UserSetSelector_Default)
	# camera.UserSetLoad()

	# Set to UserSet0 (typically where the correct camera settings are stored).
...
def ConfigureCustomImageSettings(camera, cam_params):
	...
	try:
		...
		# camera.BalanceWhiteAuto.SetValue(PySpin.BalanceWhiteAuto_Off) # When using mono cameras, there is no white balance so this line causes issues.
