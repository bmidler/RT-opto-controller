...
int num_pulses = 0; // New variable to store the number of pulses.
...
void setup( void ) {
  ...
  // Set Number of Pulses using Python or Serial Monitor input
  num_pulses = SetNumPulses();
...
int SetNumPulses() {
  // Receive number of pulses from Python or Serial Monitor.
  while (Serial.available() == 0) {}
  int pulses = int((unsigned int)Serial.parseFloat());
...
void loop( void ) {
  ...
  static int pulse_count = 0; // New variable to track the number of pulses.
  ...
  if (Serial.available())
  {
    ...
    frame_rate_out = SetFrameRate();
    frame_period = SetFramePeriod(frame_rate_out);
    num_pulses = SetNumPulses(); // Receive the number of pulses.
  ...
  if (frame_rate_out > 0) {
    ...
    // Only send the pulse if we haven't hit the number of pulses yet.
