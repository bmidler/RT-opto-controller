...
def StartTriggers(systems, params):
	try:
		...
		# Calculate the number of pulses to send.
		num_pulses = int(params["frameRate"] * params["recTimeInSec"])

		# Serialize pin length, IDs, and frame rate, (and now number of pulses) to a single string
		serialList = [len(params["digitalPins"])] + params["digitalPins"] + [params["frameRate"], num_pulses]
		...
		print("Arduino on port {} is ready to trigger pins {} at {} fps for {} pulses."\
            .format(params["serialPort"], params["digitalPins"], params["frameRate"], num_pulses), flush=True)
