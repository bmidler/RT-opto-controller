...
class VideoLogWriter:
    ...
    # -- lifecycle ----------------------------------------------------------
    def open(self, session_root: str = None, ts: str = None) -> None:
        """Create the camera's output folder, video stream, and CSV.

        `session_root` and `ts` are supplied by the controller so that every
        camera in a multi-camera session shares one timestamped parent folder
        (output_folder/<ts>_<stem>/<camera_name>/). When omitted they are
        derived here, preserving the single-camera default.
        """
        stem, ext = os.path.splitext(self.config.video_filename)
        if ts is None:
            ts = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        if session_root is None:
            session_root = os.path.join(self.config.output_folder, f"{ts}_{stem}")
        self.session_dir = os.path.join(session_root, self.config.camera_name)
    ...
    def stop(self) -> None:
        ...
        # Closing the ffmpeg pipe can raise on a real recording (broken pipe,
        # non-zero ffmpeg exit, dropped/incomplete frames). That MUST NOT
        # propagate: it would abort the rest of shutdown -- including the
        # metadata + stim CSVs written afterwards -- even though the data is
        # fine. Swallow and log it; the CSV log is closed regardless.
        try:
            if self._writer is not None:
                self._writer.close()
        except Exception as e:
            print(f"[writer] ffmpeg close failed (video may be truncated): {e}")
        finally:
            try:
                ...
                print(f"[writer] log close failed: {e}")
